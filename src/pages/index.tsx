...
import { claimRewardAll, stakeAllNft, withdrawAllNft } from "../contexts/transaction";
import {
  getGlobalData,
  getUnstakedNFTs,
  getUserPoolData,
...
export default function HomePage(props: {
  ...
  const handleStakeAll = async () => {
    ...
    try {
      startLoading();
      await stakeAllNft(wallet, map(nftList, (nft) => ({
        mint: new PublicKey(nft.mint), role: nft.role,
      })), +DEFAULT_LOCKDAY, +DEFAULT_MODEL);
  ...
  const handleUnstakeAll = async () => {
    ...
    try {
      startLoading();
      await withdrawAllNft(wallet, map(stakedNfts, (nft) => 
        new PublicKey(nft.nftAddress)
      ));
      updatePage();
